...
router.get('/filter', (req, res) => {
    ...
    if (filterLetter) {
        matchingFriends = matchingFriends.filter(friend => friend.name[0] == filterLetter.toUpperCase(''))
    }
...
let friend = friends.find(friend => friend.id == friendId)
    ...
    friend ? res.status(200).json({result: 'Finding friend with ID ' + friendId, friend})
...
router.put('/:id', (req, res) => {
    ...
    if (friendToUpdate) {
        friendToUpdate = {...friendToUpdate, ...updatedFriend}
